...
describe("artifact authorization", () => {
  ...
  it("requires and returns the exact branch and commit count for commit artifacts", async () => {
    ...
    const service = authorizer({ loadCommits });
    const controller = new AbortController();

    await expect(
      service.authorizePullCommits(input(), controller.signal),
    ...
    expect(loadCommits).toHaveBeenCalledWith(
      { number: 7, repository: "example/repo" },
      controller.signal,
    );
  });

  it.each([
    ["missing count", { commitCount: null }],
    ["negative count", { commitCount: -1 }],
    ["missing branch", { headRefName: "" }],
    ["malformed head repository", { headRepository: "../repo" }],
    ["missing cross-repository flag", { isCrossRepository: null }],
  ])("fails commit authorization closed for %s", async (_label, change) => {
    const service = authorizer({
    ...
    await expect(service.authorizePullCommits(input())).rejects.toMatchObject({
      code: "incomplete",
    });
  });
  ...
  it.each([
    ...
    "fast-denies failed-check authorization with %s expiry metadata",
    async (_label, expired) => {
      const loadCheck = vi.fn(async () => checkProof());
      const service = authorizer({
  ...
  it.each([
    ...
    "fast-denies an expired snapshot with a %s",
    async (_label, current, value, code) => {
      const loadPull = vi.fn(async () => proof());
      const service = authorizer({ current, loadPull });

      await expect(service.authorizePull(value)).rejects.toMatchObject({
        code,
      });
      expect(loadPull).not.toHaveBeenCalled();
    },
  ...
  it.each([
    [
      "different job on the same head",
      checkProof({
        failedChecks: [
          {
            checkId: "check-702",
            detailsUrl:
              "https://github.com/example/repo/actions/runs/700/job/702",
            jobId: "702",
            name: "Other",
            runId: "700",
          },
        ...
        failedChecks: [
          {
            checkId: "check-801",
            detailsUrl:
              "https://github.com/example/repo/actions/runs/800/job/801",
            jobId: "801",
            name: "Rerun",
            runId: "800",
          },
  ...
  it("rejects incomplete current check evidence", async () => {
    const service = authorizer({
      loadCheck: vi.fn(async () =>
        checkProof({ checksComplete: false, complete: false }),
      ),
    });
