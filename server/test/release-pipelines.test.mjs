...
describe("release pipelines", () => {
  ...
  it("loads release-event runs once per repository and maps exact tags and publication time", async () => {
    ...
    const result = await loadReleasePipelines({
      executor,
      now: () => NOW,
      releases: [release(), release({ id: "11", tag: "1.43.9" })],
    });
  ...
  it("polls known workflow runs through their exact endpoint", async () => {
    const item = release();
    const executor = {
      rest: vi.fn(async () =>
        run({
    ...
    const result = await loadReleasePipelines({
      exactRuns: new Map([
        [
          "appwrite-labs/cloud:10:1.43.8:2026-07-24T04:37:50.000Z",
          ["30067297377"],
        ],
      ]),
      executor,
    ...
    expect(executor.rest).toHaveBeenCalledWith(
      "repos/appwrite-labs/cloud/actions/runs/30067297377",
  ...
  it("groups discovery by repository while exact polls stay release-specific", async () => {
    const active = release();
    const discovering = release({ id: "11", tag: "1.43.9" });
    const executor = {
  ...
  it("bounds broad reads to seven days and narrowly discovers reruns for older releases", async () => {
    ...
    expect(executor.rest).toHaveBeenCalledTimes(2);
    const [broad, exact] = executor.rest.mock.calls.map(
      ([endpoint]) => new URL(endpoint, "https://api.github.com/"),
    );
  ...
  it("falls back to exact tag queries after GitHub caps a broad filtered result", async () => {
    const executor = {
      rest: vi.fn(async (endpoint) => {
        ...
        return endpoint.includes("branch=1.43.8") ? page([run()]) : page([]);
      }),
    };
  ...
  it("settles old unmatched releases even when another tag has a workflow run", async () => {
    const executor = {
      rest: vi.fn(async () => page([run({ headBranch: "1.43.7" })])),
    };
    ...
    const cases = [
      ...
      vi.fn(async (endpoint) =>
        /[?&]page=1(?:&|$)/.test(endpoint)
          ? page(
              Array.from({ length: 100 }, (_, index) =>
                run({ id: 1000 + index, workflowId: 2000 + index }),
              ),
              101,
            )
      ...
      vi.fn(async (endpoint) =>
        endpoint.includes("branch=")
          ? page(
              Array.from({ length: 100 }, (_, index) =>
                run({ id: 30_000 + index, workflowId: 40_000 + index }),
              ),
              1001,
            )
          : page(
              Array.from({ length: 100 }, (_, index) =>
                run({ id: 50_000 + index, workflowId: 60_000 + index }),
              ),
              1001,
            ),
      ),
