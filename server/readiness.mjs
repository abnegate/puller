...
function normalizeCi(ci) {
  ...
  const complete = ci?.complete === true && countsValid && stateValid && state !== 'unknown'
...
export function assessPull(pull, rank) {
  const reviewThreads = Array.isArray(pull.reviewThreads) ? pull.reviewThreads : []
  const unresolved = reviewThreads.filter((thread) => thread?.isResolved === false).length
  ...
  const baseRefOid = typeof pull.baseRefOid === 'string' ? pull.baseRefOid.toLowerCase() : ''
  const headRefOid = typeof pull.headRefOid === 'string' ? pull.headRefOid.toLowerCase() : ''
  ...
  else if (ci.state === 'unknown') blockers.push('CI checks could not be fully checked')
  ...
  const greptileComment = reviewUrl === null
    ? null
