...
const ERRORS = {
  ...
  not_found: [
    404,
    "The pull request is not available to the current GitHub user.",
  ],
  snapshot_unavailable: [
    503,
    "A complete current authored pull request snapshot is required.",
  ],
...
function snapshotPull(peek, input) {
  ...
  const matches = [...snapshot.ready, ...snapshot.notReady].filter(
    (pull) =>
      isRecord(pull) &&
...
function validateProof(proof, input, snapshotViewer) {
  ...
  if (
    ...
    (input.expectedViewerLogin !== null &&
      proof.viewerLogin.toLowerCase() !==
        input.expectedViewerLogin.toLowerCase())
  ) {
    throw new AuthorizationError("not_found");
  }
  if (
...
function validateCommitProof(proof, input, snapshotViewer) {
  const authorization = validateProof(proof, input, snapshotViewer);
  if (
    typeof proof.headRefName !== "string" ||
    proof.headRefName.trim() === "" ||
    typeof proof.headRepository !== "string" ||
    !REPOSITORY.test(proof.headRepository) ||
    proof.headRepository
      .split("/")
      .some((part) => part === "." || part === "..") ||
    typeof proof.isCrossRepository !== "boolean" ||
    !Number.isSafeInteger(proof.commitCount) ||
    proof.commitCount < 0
  ) {
    throw new AuthorizationError("incomplete");
  }
  ...
  return Object.freeze({
    ...
    isCrossRepository: proof.isCrossRepository,
  });
}

export function createArtifactAuthorizer({
  loadCheckAuthorization,
  ...
  if (typeof loadPullCommitsAuthorization !== "function") {
    throw new TypeError("loadPullCommitsAuthorization must be a function.");
  }
  if (typeof peek !== "function")
    throw new TypeError("peek must be a function.");
  ...
  async function authorizeFailedCheck(value, signal) {
    ...
    const matches = proof.failedChecks.filter(
      (check) =>
        isRecord(check) &&
  ...
  async function authorizePullCommits(value, signal) {
    const input = validateInput(value, false);
    const snapshotViewer = snapshotPull(peek, input);
    const proof = await loadProof(
      loadPullCommitsAuthorization,
    ...
    return validateCommitProof(proof, input, snapshotViewer);
  }

  return Object.freeze({
    authorizeFailedCheck,
    authorizePull,
    authorizePullCommits,
  });
}
