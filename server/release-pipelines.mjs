...
export function mergeReleasePipeline(previous, incoming) {
  ...
  const lookup = unavailable
    ? previous.lookup === 'pending' ? 'pending' : 'unavailable'
...
async function loadPages(executor, repository, created, branch) {
  ...
  for (let page = 1; page <= pages; page += 1) {
    ...
    const expected = page < pages
      ? PAGE_SIZE
      : first.total_count - PAGE_SIZE * (pages - 1)
...
function available(release, runs, checkedAt, now) {
  ...
  const discovering =
    ...
    RELEASE_REFRESH_DISCOVERY_WINDOW
  return {
    id: release.id,
    pipeline: {
      checkedAt,
...
async function loadRepository(executor, releases, previous, checkedAt, now) {
  ...
  const earliest = releases.reduce(
    (value, release) =>
      Math.min(value, Date.parse(release.publishedAt)),
  ...
  try {
    ...
    const broadRuns = broad.runs.map((value) => normalizeRun(value, repository))
    if (broadRuns.some((value) => value === null)) {
      throw new Error('GitHub Actions returned malformed release workflow evidence.')
    }
    if (!broad.capped) {
...
export async function loadReleasePipelines({
  ...
  if (!Array.isArray(releases) || releases.some((release) => !validRelease(release))) {
  ...
  const values = await mapLimit(
    [...groups.values()],
    READ_CONCURRENCY,
    (group) => loadRepository(executor, group, prior, checkedAt, checked),
  )
  const byRelease = new Map(values
    .flat()
    .map((release) => [releasePipelineKey(release), release]))
  return {
